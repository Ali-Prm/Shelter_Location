...
def trial_error_model(scenario, demand, od, candidate, nearest_candid,
                      max_shelter_num, shelter_cap,
                      gamma):
    """
    Trial-and-Error Model Generator
    Input:
    ...
    Output:
    A object of the trial-and-error model 
    ...
    model = pyo.ConcreteModel('trial-and-error')
    ...
    # Sorted list of each node neighboring candidate sorted based on people's trial-and-error behavior
    ...
    # flow variable between demand nodes and shelter locations : nonnegative variable
    model.y = pyo.Var(model.I, model.J, within=pyo.NonNegativeReals)

    # unresponded demand from each node : nonnegative variable 
    ...
    # travel time 
    model.tt = pyo.Param(model.I, model.J, initialize= od[scenario])
